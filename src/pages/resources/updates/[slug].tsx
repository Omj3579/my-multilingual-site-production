...
const DynamicUpdatesPage = () => {
      ...
      try {
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            const posts = Array.isArray(apiData) ? apiData : apiData.updates || apiData.posts || [];
        ...
        convertedPost.category = 'updates';
  ...
  async function fetchRelatedPosts(tags: string[], currentSlug: string) {
    try {
      ...
      const response = await fetch(`/api/combined-updates`);
      if (response.ok) {
        const data = await response.json();
        const posts = Array.isArray(data) ? data : data.updates || data.posts || [];
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center max-w-md mx-auto p-6">
            ...
            <p className="text-gray-600 mb-6">
              The requested updates could not be found or may have been moved.
            </p>
            <Link
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <nav className="flex items-center space-x-2 text-sm">
              ...
              <Link href={`/resources/updates`} className="text-gray-500 hover:text-gray-700 capitalize">
                {category.replace('-', ' ')}
              </Link>
              <span className="text-gray-400">/</span>
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                    ...
                    <div className="space-y-4">
                      {relatedPosts.map((post) => (
                        <Link
                          key={post.slug}
                          href={`/resources/updates/${post.slug}`}
...
export default DynamicUpdatesPage;
