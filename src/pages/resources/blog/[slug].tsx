...
const DynamicBlogPage = () => {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        ...
        const sources = [
          `/api/combined-blog-posts`,
          `/posts/blog/${slug}/data.json`,
        ];
        ...
        for (const source of sources) {
          try {
            const apiRes = await fetch(source);
            if (apiRes.ok) {
              const apiData = await apiRes.json();
              const posts = Array.isArray(apiData) ? apiData : apiData.blog || apiData.posts || [];
  ...
  return (
    <>
      <Head>
        <title>{blogPost.title[language]} | Blog</title>
        <meta name="description" content={blogPost.excerpt[language]} />
        ...
        <meta property="og:description" content={blogPost.excerpt[language]} />
        {blogPost.image && <meta property="og:image" content={blogPost.image} />}
        <meta property="og:type" content="article" />
        <meta name="author" content={blogPost.author.name[language]} />
        <meta name="article:published_time" content={blogPost.publishedAt} />
      ...
      <ResourcesLayout>
        <BlogPostTemplate post={blogPost} language={language} />
      </ResourcesLayout>
    </>
  );
};
  ...
  async function fetchRelatedPosts(tags: string[], currentSlug: string) {
    try {
      ...
      if (response.ok) {
        ...
        const posts = Array.isArray(data) ? data : data.blog || data.posts || [];
