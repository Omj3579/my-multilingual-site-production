      ...
      <div className="relative z-10 px-6 py-20 md:py-32">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 items-start gap-8 lg:gap-16">
            ...
            >
              ...
              >
                <motion.a
                  ...
                  className="inline-flex items-center gap-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white py-3 px-6 rounded-lg font-medium shadow-lg overflow-hidden relative group no-underline"
            ...
            {/* Right image section with modern presentation */}
            <motion.div
              ref={imageRef}
              ...
              transition={{ duration: 0.8 }}
            >
              {/* Modern image container with geometric elements */}
              <motion.div
              ...
              >
                ...
                <div className="absolute -inset-8 overflow-hidden pointer-events-none">
                  <motion.div 
                    className="absolute top-0 right-0 w-24 h-24 border-2 border-blue-200 rounded-lg rotate-12"
                    animate={{ rotate: [12, 22, 12] }}
                    transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                  />
                  <motion.div 
                    className="absolute bottom-4 left-4 w-16 h-16 bg-gradient-to-br from-amber-200 to-amber-300 rounded-full opacity-60"
                    animate={{ scale: [1, 1.1, 1] }}
                    transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                  />
                  <div className="absolute top-1/2 left-0 w-2 h-32 bg-gradient-to-b from-blue-300 to-transparent transform -translate-y-1/2 -translate-x-4" />
                </div>
                ...
                >
                    ...
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-slate-700">
                          {language === 'en' ? 'IMD Active' : 'IMD Aktív'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 text-center">
                    <h4 className="font-semibold text-slate-800 mb-2">
                      {language === 'en' ? 'Advanced IMD Process' : 'Fejlett IMD Folyamat'}
                    </h4>
                ...
                {/* Floating tech specs */}
                <motion.div
                  className="absolute -right-8 top-1/3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg p-3 shadow-lg max-w-[140px]"
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: imageInView ? 1 : 0, x: imageInView ? 0 : 20 }}
                  transition={{ duration: 0.7, delay: 0.6 }}
                >
                  <div className="text-xs font-medium mb-1">
                    {language === 'en' ? 'Durability' : 'Tartósság'}
                  </div>
