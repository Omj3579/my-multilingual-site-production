...
import { Leaf, ArrowRight, Globe, Recycle, Users, Target } from 'lucide-react';

const SustainabilityPathwaySection = () => {
  const { language } = useLanguage();
  
  // Define pathway steps in sequence
  const pathwaySteps = [
    {
      icon: Users,
      title: language === 'en' ? 'Persistent Collaboration' : 'Folyamatos Együttműködés',
      description: language === 'en'
        ? 'We engage in an ongoing dialogue with our customers and suppliers, reaching out to both local and broader communities.'
        : 'Folyamatos párbeszédet folytatunk ügyfeleinkkel és beszállítóinkkal, kapcsolatot tartva mind a helyi, mind a szélesebb közösségekkel.'
    },
    {
      icon: Recycle,
      title: language === 'en' ? 'Eco-Conscious Design' : 'Környezettudatos Tervezés',
      description: language === 'en'
        ? 'We meticulously craft our processes to prioritize environmental sustainability, thoroughly reviewing the ecological implications of each decision.'
        : 'Gondosan alakítjuk folyamatainkat a környezeti fenntarthatóság előtérbe helyezésével, alaposan áttekintve minden döntésünk ökológiai következményeit.'
    },
    {
      icon: Target,
      title: language === 'en' ? 'Zero Impact Mission' : 'Nulla Hatás Küldetés',
      description: language === 'en'
        ? 'Our goal is to minimize our environmental footprint by enhancing our input-output equilibrium, ensuring optimal value without leaving a trace.'
        : 'Célunk a környezeti lábnyomunk minimalizálása a bemenet-kimenet egyensúly javításával, biztosítva az optimális értéket nyom nélkül.'
    }
  ];
            ...
            >
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-800 mb-6 leading-tight">
                <span className="text-green-600 italic font-medium mr-2">
                  {language === 'en' ? 'Sustainability' : 'Fenntarthatósági'}
                </span>
                <br />
                {language === 'en' ? 'Pathway and Aspirations' : 'Utunk és Törekvéseink'}
              </h2>
              
              <p className="text-lg text-gray-600 max-w-lg">
                {language === 'en' 
                  ? "Our journey toward sustainability is guided by clear principles that shape every decision we make and every product we create."
                  : "Fenntarthatóság felé vezető utunkat olyan egyértelmű elvek vezérlik, amelyek minden döntésünket és minden általunk létrehozott terméket formálnak."}
              </p>
            </motion.div>
