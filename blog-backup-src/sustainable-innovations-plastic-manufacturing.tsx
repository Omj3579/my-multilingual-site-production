...
const SustainableInnovationsPlasticManufacturingPost = () => {
  const { language } = useLanguage();
  
  // Get the blog post data
  const blogPost = getBlogPostBySlug('sustainable-innovations-plastic-manufacturing');
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:type" content="article" />        <meta name="author" content={blogPost.author.name} />
      ...
      <ResourcesLayout>
        <article className="max-w-4xl mx-auto px-4 py-8">
          ...
          <header className="mb-8">
            ...
            {/* Meta Information */}
            <div className="flex flex-wrap items-center gap-6 text-sm text-gray-500 mb-6">              <div className="flex items-center gap-2">
                <User className="h-4 w-4" />
                <span>{blogPost.author.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4" />
                <span>{new Date(blogPost.date).toLocaleDateString(language)}</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="bg-gray-50 rounded-xl p-6 mb-8">
            <div className="flex items-start gap-4">              {blogPost.author.avatar && (
          ...
          <div className="pt-8 border-t border-gray-200">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm text-gray-500">
                Published on {new Date(blogPost.date).toLocaleDateString(language)}
              </div>
            </div>
          </div>
        </article>
...
export default SustainableInnovationsPlasticManufacturingPost;
